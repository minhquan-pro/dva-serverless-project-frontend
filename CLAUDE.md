...
## Bối cảnh dự án
Frontend cho website bán đồ ăn sáng của nhà (bánh cuốn, bún chả, ...). Đây là phần frontend của "dva-serverless-project" — backend dự kiến là AWS serverless (API Gateway + Lambda), giao tiếp qua REST API bằng Axios. Dự án đã được scaffold (Vite + React + TypeScript + Tailwind v4 + Axios + React Router), nhiều trang riêng biệt, có form liên hệ và đăng nhập bằng Google (chưa có giỏ hàng/đặt món online — tài khoản là bước chuẩn bị cho tính năng đó).

## Router & xác thực (đã chốt qua brainstorming)
- **Nhiều trang riêng biệt** (không phải one-page nữa): `/` (Trang chủ), `/thuc-don`, `/thuc-don/:id` (chi tiết món), `/gioi-thieu`, `/lien-he`, `/dang-nhap`, `/tai-khoan` (route bảo vệ, redirect về `/dang-nhap` nếu chưa đăng nhập qua `ProtectedRoute`). `Layout` (Header + `<Outlet/>` + Footer) bọc toàn bộ route qua `App.tsx`.
...
- Trang Tài khoản (`AccountPage.tsx`, route `/tai-khoan`) hiển thị avatar/tên/email từ Google + form SĐT/địa chỉ giao hàng (1 địa chỉ duy nhất) có nút "Lưu thông tin". Trang chi tiết món (`ProductPage.tsx`) đã có nút "Thêm vào giỏ hàng" nhưng cố tình vô hiệu hoá (`disabled`, kèm ghi chú gọi điện đặt trước) — CHƯA có giỏ hàng/đặt món thật. Khi làm tính năng đặt món online thật, brainstorm lại phạm vi trước khi thêm.
