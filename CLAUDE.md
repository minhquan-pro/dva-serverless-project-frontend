...
- **Nhiều trang riêng biệt** (không phải one-page nữa): `/` (Trang chủ), `/thuc-don`, `/thuc-don/:id` (chi tiết món), `/gioi-thieu`, `/lien-he`, `/dang-nhap`, `/dang-ky`, `/tai-khoan` (route bảo vệ, redirect về `/dang-nhap` nếu chưa đăng nhập qua `ProtectedRoute`). `Layout` (Header + `<Outlet/>` + Footer) bọc toàn bộ route qua `App.tsx`.
...
- Tài khoản hiện chỉ lưu `name`/`phone` — chưa gắn với giỏ hàng/đơn hàng. Trang chi tiết món (`ProductPage.tsx`) đã có nút "Thêm vào giỏ hàng" nhưng cố tình vô hiệu hoá (`disabled`, kèm ghi chú gọi điện đặt trước) — CHƯA có giỏ hàng/đặt món thật. Khi làm tính năng đặt món online thật, brainstorm lại phạm vi trước khi thêm.
